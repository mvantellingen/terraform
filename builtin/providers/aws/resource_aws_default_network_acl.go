...
func resourceAwsDefaultNetworkAcl() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"ingress_deny": &schema.Schema{
				Type:          schema.TypeBool,
				Optional:      true,
				Default:       false,
				Description:   "Deny all Ingress connections. Default false",
				ConflictsWith: []string{"ingress"},
			},
			"egress_deny": &schema.Schema{
				Type:          schema.TypeBool,
				Optional:      true,
				Default:       false,
				Description:   "Deny all Egress connections. Default false",
				ConflictsWith: []string{"egress"},
			},
			"tags": tagsSchema(),
		},
	}
...
func resourceAwsDefaultNetworkAclCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.Get("ingress_deny").(bool) {
		err := revokeRulesForType(d.Id(), "ingress", meta)
		if err != nil {
			return err
		}
	}
	if d.Get("egress_deny").(bool) {
